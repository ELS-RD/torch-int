...
def test_quant_linear_a8_w8_b32_o32():
    ...
    bias = torch.randint(torch.iinfo(torch.int32).min, torch.iinfo(torch.int32).max, (N,), dtype=torch.int32)
    ...
    assert torch.allclose(y_gt, y.float().cpu(), atol=1e-3)
    y_triton = torch.zeros_like(y)
    y_triton = matmul_quant(x.cuda(), weight.cuda().t(), bias.cuda(), 1., 1., y_triton.cuda())
...
def test_quant_linear_a8_w8_b32_o32_with_scaling():
    ...
    y_gt = linear(x.float())
    y = linear_a8_w8_b32_o32_with_scaling(x, weight, bias, alpha, beta)
